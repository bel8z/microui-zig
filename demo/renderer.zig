const std = @import("std");
const mu = @import("microui");
const assert = std.assert;

const c = @cImport({
    @cInclude("SDL2/SDL_opengl.h");
});

const Renderer = @This();

const buffer_size = 16384;

var tex_buf: f32[buffer_size * 8] = undefined;
var vert_buf: f32[buffer_size * 8] = undefined;
var color_buf: u8[buffer_size * 16] = undefined;
var index_buf: c_uint[buffer_size * 6] = undefined;
var init_done = std.atomic.Atomic(bool).init(false);

width: c_int,
height: c_int,
buf_idx: c_uint = 0,

pub fn init(width: c_int, height: c_int) Renderer {
    while (true) {
        if (init_done.compareAndSwap(false, true, .SeqCst, .SeqCst)) |value| {
            assert(value);

            // init gl
            c.glEnable(c.GL_BLEND);
            c.glBlendFunc(c.GL_SRC_ALPHA, c.GL_ONE_MINUS_SRC_ALPHA);
            c.glDisable(c.GL_CULL_FACE);
            c.glDisable(c.GL_DEPTH_TEST);
            c.glEnable(c.GL_SCISSOR_TEST);
            c.glEnable(c.GL_TEXTURE_2D);
            c.glEnableClientState(c.GL_VERTEX_ARRAY);
            c.glEnableClientState(c.GL_TEXTURE_COORD_ARRAY);
            c.glEnableClientState(c.GL_COLOR_ARRAY);

            // init texture
            var id: c_uint = undefined;
            c.glGenTextures(1, &id);
            c.glBindTexture(c.GL_TEXTURE_2D, id);
            c.glTexImage2D(c.GL_TEXTURE_2D, 0, c.GL_ALPHA, ATLAS_WIDTH, ATLAS_HEIGHT, 0, c.GL_ALPHA, c.GL_UNSIGNED_BYTE, atlas_texture);
            c.glTexParameteri(c.GL_TEXTURE_2D, c.GL_TEXTURE_MIN_FILTER, c.GL_NEAREST);
            c.glTexParameteri(c.GL_TEXTURE_2D, c.GL_TEXTURE_MAG_FILTER, c.GL_NEAREST);
            assert(c.glGetError() == 0);

            return .{ .width = width, .heigth = height };
        }
    }
}

pub fn flush(self: *Renderer) void {
    if (self.buf_idx == 0) return;

    c.glViewport(0, 0, self.width, self.height);
    c.glMatrixMode(c.GL_PROJECTION);
    c.glPushMatrix();
    c.glLoadIdentity();
    c.glOrtho(0.0, self.width, self.height, 0.0, -1.0, 1.0);
    c.glMatrixMode(c.GL_MODELVIEW);
    c.glPushMatrix();
    c.glLoadIdentity();

    c.glTexCoordPointer(2, c.GL_FLOAT, 0, tex_buf);
    c.glVertexPointer(2, c.GL_FLOAT, 0, vert_buf);
    c.glColorPointer(4, c.GL_UNSIGNED_BYTE, 0, color_buf);
    c.glDrawElements(c.GL_TRIANGLES, self.buf_idx * 6, c.GL_UNSIGNED_INT, index_buf);

    c.glMatrixMode(c.GL_MODELVIEW);
    c.glPopMatrix();
    c.glMatrixMode(c.GL_PROJECTION);
    c.glPopMatrix();

    self.buf_idx = 0;
}

pub fn drawRect(self: *Renderer, rect: mu.Rect, color: mu.Color) void {
    self.pushQuad(rect, atlas[ATLAS_WHITE], color);
}

pub fn drawIcon(self: *Renderer, id: mu.Icon, rect: mu.Rect, color: mu.Color) void {
    const index = @enumToInt(id);

    if (index >= ATLAS_WHITE) {
        assert(false);
        return;
    }

    const src = atlas[index];
    var dst = mu.Rect{ .pt = rect.pt, .sz = src.sz };
    dst.pt.x += (rect.sz.x - dst.sz.x) / 2;
    dst.pt.y += (rect.sz.y - dst.sz.y) / 2;

    self.pushQuad(dst, src, color);
}

// pub fn drawText(self: *Renderer, text: []const u8, pos: mu.Vec2, color: mu.Color) void;

// pub fn getTextWidth(self: *Renderer, text: []const u8) i32;

pub fn getTextHeight(self: *Renderer) i32 {
    _ = self;
    return 18;
}

pub fn setClipRect(self: *Renderer, rect: mu.Rect) void {
    self.flush();
    c.glScissor(
        rect.pt.x,
        self.height - (rect.pt.y + rect.sz.y),
        rect.sz.x,
        rect.sz.y,
    );
}

pub fn clear(self: *Renderer, color: mu.Color) void {
    self.flush();
    c.glClearColor(
        @intToFloat(f32, color.r / 255),
        @intToFloat(f32, color.g / 255),
        @intToFloat(f32, color.b / 255),
        @intToFloat(f32, color.a / 255),
    );
    c.glClear(c.GL_COLOR_BUFFER_BIT);
}

fn pushQuad(self: *Renderer, dst: mu.Rect, src: mu.Rect, color: mu.Color) void {
    if (self.buf_idx == buffer_size) {
        self.flush();
    }

    const texvert_idx = self.buf_idx * 8;
    const color_idx = self.buf_idx * 16;
    const element_idx = self.buf_idx * 4;
    const index_idx = self.buf_idx * 6;

    self.buf_idx += 1;

    // update texture buffer
    const atlas_w = @intToFloat(f32, ATLAS_WIDTH);
    const atlas_h = @intToFloat(f32, ATLAS_HEIGHT);
    const x = @intToFloat(f32, src.x) / atlas_w;
    const y = @intToFloat(f32, src.y) / atlas_h;
    const w = @intToFloat(f32, src.w) / atlas_w;
    const h = @intToFloat(f32, src.h) / atlas_h;
    tex_buf[texvert_idx + 0] = x;
    tex_buf[texvert_idx + 1] = y;
    tex_buf[texvert_idx + 2] = x + w;
    tex_buf[texvert_idx + 3] = y;
    tex_buf[texvert_idx + 4] = x;
    tex_buf[texvert_idx + 5] = y + h;
    tex_buf[texvert_idx + 6] = x + w;
    tex_buf[texvert_idx + 7] = y + h;

    // update vertex buffer
    vert_buf[texvert_idx + 0] = dst.x;
    vert_buf[texvert_idx + 1] = dst.y;
    vert_buf[texvert_idx + 2] = dst.x + dst.w;
    vert_buf[texvert_idx + 3] = dst.y;
    vert_buf[texvert_idx + 4] = dst.x;
    vert_buf[texvert_idx + 5] = dst.y + dst.h;
    vert_buf[texvert_idx + 6] = dst.x + dst.w;
    vert_buf[texvert_idx + 7] = dst.y + dst.h;

    // update color buffer
    memcpy(color_buf + color_idx + 0, &color, 4);
    memcpy(color_buf + color_idx + 4, &color, 4);
    memcpy(color_buf + color_idx + 8, &color, 4);
    memcpy(color_buf + color_idx + 12, &color, 4);

    // update index buffer
    index_buf[index_idx + 0] = element_idx + 0;
    index_buf[index_idx + 1] = element_idx + 1;
    index_buf[index_idx + 2] = element_idx + 2;
    index_buf[index_idx + 3] = element_idx + 2;
    index_buf[index_idx + 4] = element_idx + 3;
    index_buf[index_idx + 5] = element_idx + 1;
}

const ATLAS_WHITE = @enumToInt(mu.Icon.Expanded) + 1;
const ATLAS_FONT = ATLAS_WHITE + 1;
const ATLAS_WIDTH = 128;
const ATLAS_HEIGHT = 128;

const atlas = init: {
    var value: [134]mu.Rect = undefined;
    value[@enumToInt(mu.Icon.Close)] = mu.Rect.init(88, 68, 16, 16);
    value[@enumToInt(mu.Icon.Check)] = mu.Rect.init(0, 0, 18, 18);
    value[@enumToInt(mu.Icon.Collapsed)] = mu.Rect.init(113, 68, 5, 7);
    value[@enumToInt(mu.Icon.Expanded)] = mu.Rect.init(118, 68, 7, 5);
    value[ATLAS_WHITE] = mu.Rect.init(125, 68, 3, 3);
    value[ATLAS_FONT + 32] = mu.Rect.init(84, 68, 2, 17);
    value[ATLAS_FONT + 33] = mu.Rect.init(39, 68, 3, 17);
    value[ATLAS_FONT + 34] = mu.Rect.init(114, 51, 5, 17);
    value[ATLAS_FONT + 35] = mu.Rect.init(34, 17, 7, 17);
    value[ATLAS_FONT + 36] = mu.Rect.init(28, 34, 6, 17);
    value[ATLAS_FONT + 37] = mu.Rect.init(58, 0, 9, 17);
    value[ATLAS_FONT + 38] = mu.Rect.init(103, 0, 8, 17);
    value[ATLAS_FONT + 39] = mu.Rect.init(86, 68, 2, 17);
    value[ATLAS_FONT + 40] = mu.Rect.init(42, 68, 3, 17);
    value[ATLAS_FONT + 41] = mu.Rect.init(45, 68, 3, 17);
    value[ATLAS_FONT + 42] = mu.Rect.init(34, 34, 6, 17);
    value[ATLAS_FONT + 43] = mu.Rect.init(40, 34, 6, 17);
    value[ATLAS_FONT + 44] = mu.Rect.init(48, 68, 3, 17);
    value[ATLAS_FONT + 45] = mu.Rect.init(51, 68, 3, 17);
    value[ATLAS_FONT + 46] = mu.Rect.init(54, 68, 3, 17);
    value[ATLAS_FONT + 47] = mu.Rect.init(124, 34, 4, 17);
    value[ATLAS_FONT + 48] = mu.Rect.init(46, 34, 6, 17);
    value[ATLAS_FONT + 49] = mu.Rect.init(52, 34, 6, 17);
    value[ATLAS_FONT + 50] = mu.Rect.init(58, 34, 6, 17);
    value[ATLAS_FONT + 51] = mu.Rect.init(64, 34, 6, 17);
    value[ATLAS_FONT + 52] = mu.Rect.init(70, 34, 6, 17);
    value[ATLAS_FONT + 53] = mu.Rect.init(76, 34, 6, 17);
    value[ATLAS_FONT + 54] = mu.Rect.init(82, 34, 6, 17);
    value[ATLAS_FONT + 55] = mu.Rect.init(88, 34, 6, 17);
    value[ATLAS_FONT + 56] = mu.Rect.init(94, 34, 6, 17);
    value[ATLAS_FONT + 57] = mu.Rect.init(100, 34, 6, 17);
    value[ATLAS_FONT + 58] = mu.Rect.init(57, 68, 3, 17);
    value[ATLAS_FONT + 59] = mu.Rect.init(60, 68, 3, 17);
    value[ATLAS_FONT + 60] = mu.Rect.init(106, 34, 6, 17);
    value[ATLAS_FONT + 61] = mu.Rect.init(112, 34, 6, 17);
    value[ATLAS_FONT + 62] = mu.Rect.init(118, 34, 6, 17);
    value[ATLAS_FONT + 63] = mu.Rect.init(119, 51, 5, 17);
    value[ATLAS_FONT + 64] = mu.Rect.init(18, 0, 10, 17);
    value[ATLAS_FONT + 65] = mu.Rect.init(41, 17, 7, 17);
    value[ATLAS_FONT + 66] = mu.Rect.init(48, 17, 7, 17);
    value[ATLAS_FONT + 67] = mu.Rect.init(55, 17, 7, 17);
    value[ATLAS_FONT + 68] = mu.Rect.init(111, 0, 8, 17);
    value[ATLAS_FONT + 69] = mu.Rect.init(0, 35, 6, 17);
    value[ATLAS_FONT + 70] = mu.Rect.init(6, 35, 6, 17);
    value[ATLAS_FONT + 71] = mu.Rect.init(119, 0, 8, 17);
    value[ATLAS_FONT + 72] = mu.Rect.init(18, 17, 8, 17);
    value[ATLAS_FONT + 73] = mu.Rect.init(63, 68, 3, 17);
    value[ATLAS_FONT + 74] = mu.Rect.init(66, 68, 3, 17);
    value[ATLAS_FONT + 75] = mu.Rect.init(62, 17, 7, 17);
    value[ATLAS_FONT + 76] = mu.Rect.init(12, 51, 6, 17);
    value[ATLAS_FONT + 77] = mu.Rect.init(28, 0, 10, 17);
    value[ATLAS_FONT + 78] = mu.Rect.init(67, 0, 9, 17);
    value[ATLAS_FONT + 79] = mu.Rect.init(76, 0, 9, 17);
    value[ATLAS_FONT + 80] = mu.Rect.init(69, 17, 7, 17);
    value[ATLAS_FONT + 81] = mu.Rect.init(85, 0, 9, 17);
    value[ATLAS_FONT + 82] = mu.Rect.init(76, 17, 7, 17);
    value[ATLAS_FONT + 83] = mu.Rect.init(18, 51, 6, 17);
    value[ATLAS_FONT + 84] = mu.Rect.init(24, 51, 6, 17);
    value[ATLAS_FONT + 85] = mu.Rect.init(26, 17, 8, 17);
    value[ATLAS_FONT + 86] = mu.Rect.init(83, 17, 7, 17);
    value[ATLAS_FONT + 87] = mu.Rect.init(38, 0, 10, 17);
    value[ATLAS_FONT + 88] = mu.Rect.init(90, 17, 7, 17);
    value[ATLAS_FONT + 89] = mu.Rect.init(30, 51, 6, 17);
    value[ATLAS_FONT + 90] = mu.Rect.init(36, 51, 6, 17);
    value[ATLAS_FONT + 91] = mu.Rect.init(69, 68, 3, 17);
    value[ATLAS_FONT + 92] = mu.Rect.init(124, 51, 4, 17);
    value[ATLAS_FONT + 93] = mu.Rect.init(72, 68, 3, 17);
    value[ATLAS_FONT + 94] = mu.Rect.init(42, 51, 6, 17);
    value[ATLAS_FONT + 95] = mu.Rect.init(15, 68, 4, 17);
    value[ATLAS_FONT + 96] = mu.Rect.init(48, 51, 6, 17);
    value[ATLAS_FONT + 97] = mu.Rect.init(54, 51, 6, 17);
    value[ATLAS_FONT + 98] = mu.Rect.init(97, 17, 7, 17);
    value[ATLAS_FONT + 99] = mu.Rect.init(0, 52, 5, 17);
    value[ATLAS_FONT + 100] = mu.Rect.init(104, 17, 7, 17);
    value[ATLAS_FONT + 101] = mu.Rect.init(60, 51, 6, 17);
    value[ATLAS_FONT + 102] = mu.Rect.init(19, 68, 4, 17);
    value[ATLAS_FONT + 103] = mu.Rect.init(66, 51, 6, 17);
    value[ATLAS_FONT + 104] = mu.Rect.init(111, 17, 7, 17);
    value[ATLAS_FONT + 105] = mu.Rect.init(75, 68, 3, 17);
    value[ATLAS_FONT + 106] = mu.Rect.init(78, 68, 3, 17);
    value[ATLAS_FONT + 107] = mu.Rect.init(72, 51, 6, 17);
    value[ATLAS_FONT + 108] = mu.Rect.init(81, 68, 3, 17);
    value[ATLAS_FONT + 109] = mu.Rect.init(48, 0, 10, 17);
    value[ATLAS_FONT + 110] = mu.Rect.init(118, 17, 7, 17);
    value[ATLAS_FONT + 111] = mu.Rect.init(0, 18, 7, 17);
    value[ATLAS_FONT + 112] = mu.Rect.init(7, 18, 7, 17);
    value[ATLAS_FONT + 113] = mu.Rect.init(14, 34, 7, 17);
    value[ATLAS_FONT + 114] = mu.Rect.init(23, 68, 4, 17);
    value[ATLAS_FONT + 115] = mu.Rect.init(5, 52, 5, 17);
    value[ATLAS_FONT + 116] = mu.Rect.init(27, 68, 4, 17);
    value[ATLAS_FONT + 117] = mu.Rect.init(21, 34, 7, 17);
    value[ATLAS_FONT + 118] = mu.Rect.init(78, 51, 6, 17);
    value[ATLAS_FONT + 119] = mu.Rect.init(94, 0, 9, 17);
    value[ATLAS_FONT + 120] = mu.Rect.init(84, 51, 6, 17);
    value[ATLAS_FONT + 121] = mu.Rect.init(90, 51, 6, 17);
    value[ATLAS_FONT + 122] = mu.Rect.init(10, 68, 5, 17);
    value[ATLAS_FONT + 123] = mu.Rect.init(31, 68, 4, 17);
    value[ATLAS_FONT + 124] = mu.Rect.init(96, 51, 6, 17);
    value[ATLAS_FONT + 125] = mu.Rect.init(35, 68, 4, 17);
    value[ATLAS_FONT + 126] = mu.Rect.init(102, 51, 6, 17);
    value[ATLAS_FONT + 127] = mu.Rect.init(108, 51, 6, 17);
    break :init value;
};

const atlas_texture: [ATLAS_WIDTH * ATLAS_HEIGHT]u8 = [_]u8{
    ,
};
