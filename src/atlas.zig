const std = @import("std");
const assert = std.debug.assert;

const mu = @import("microui.zig");

pub const width = 128;
pub const height = 128;
pub const white = mu.Rect.init(125, 68, 3, 3);
pub const text_height = 18;

const font_start = @enumToInt(mu.Icon.Expanded) + 1;

pub inline fn fullWhite() mu.Rect {
    return atlas[white];
}

pub fn getIcon(id: mu.Icon) mu.Rect {
    const index = @enumToInt(id);

    if (index >= font_start) {
        assert(false);
        return white;
    }

    return atlas[index];
}

pub fn getGlyph(char: u8) mu.Rect {
    if (char & 0xC0 == 0x80) return .{};

    const index = char + font_start;
    if (index > atlas.len) return .{};

    return atlas[index];
}

pub fn getTextWidth(text: []const u8) i32 {
    var w: i32 = 0;
    for (text) |char| w += getGlyph(char).sz.x;
    return w;
}

const atlas = init: {
    var value: [133]mu.Rect = undefined;
    value[@enumToInt(mu.Icon.Close)] = mu.Rect.init(88, 68, 16, 16);
    value[@enumToInt(mu.Icon.Check)] = mu.Rect.init(0, 0, 18, 18);
    value[@enumToInt(mu.Icon.Collapsed)] = mu.Rect.init(113, 68, 5, 7);
    value[@enumToInt(mu.Icon.Expanded)] = mu.Rect.init(118, 68, 7, 5);
    value[font_start + 32] = mu.Rect.init(84, 68, 2, 17);
    value[font_start + 33] = mu.Rect.init(39, 68, 3, 17);
    value[font_start + 34] = mu.Rect.init(114, 51, 5, 17);
    value[font_start + 35] = mu.Rect.init(34, 17, 7, 17);
    value[font_start + 36] = mu.Rect.init(28, 34, 6, 17);
    value[font_start + 37] = mu.Rect.init(58, 0, 9, 17);
    value[font_start + 38] = mu.Rect.init(103, 0, 8, 17);
    value[font_start + 39] = mu.Rect.init(86, 68, 2, 17);
    value[font_start + 40] = mu.Rect.init(42, 68, 3, 17);
    value[font_start + 41] = mu.Rect.init(45, 68, 3, 17);
    value[font_start + 42] = mu.Rect.init(34, 34, 6, 17);
    value[font_start + 43] = mu.Rect.init(40, 34, 6, 17);
    value[font_start + 44] = mu.Rect.init(48, 68, 3, 17);
    value[font_start + 45] = mu.Rect.init(51, 68, 3, 17);
    value[font_start + 46] = mu.Rect.init(54, 68, 3, 17);
    value[font_start + 47] = mu.Rect.init(124, 34, 4, 17);
    value[font_start + 48] = mu.Rect.init(46, 34, 6, 17);
    value[font_start + 49] = mu.Rect.init(52, 34, 6, 17);
    value[font_start + 50] = mu.Rect.init(58, 34, 6, 17);
    value[font_start + 51] = mu.Rect.init(64, 34, 6, 17);
    value[font_start + 52] = mu.Rect.init(70, 34, 6, 17);
    value[font_start + 53] = mu.Rect.init(76, 34, 6, 17);
    value[font_start + 54] = mu.Rect.init(82, 34, 6, 17);
    value[font_start + 55] = mu.Rect.init(88, 34, 6, 17);
    value[font_start + 56] = mu.Rect.init(94, 34, 6, 17);
    value[font_start + 57] = mu.Rect.init(100, 34, 6, 17);
    value[font_start + 58] = mu.Rect.init(57, 68, 3, 17);
    value[font_start + 59] = mu.Rect.init(60, 68, 3, 17);
    value[font_start + 60] = mu.Rect.init(106, 34, 6, 17);
    value[font_start + 61] = mu.Rect.init(112, 34, 6, 17);
    value[font_start + 62] = mu.Rect.init(118, 34, 6, 17);
    value[font_start + 63] = mu.Rect.init(119, 51, 5, 17);
    value[font_start + 64] = mu.Rect.init(18, 0, 10, 17);
    value[font_start + 65] = mu.Rect.init(41, 17, 7, 17);
    value[font_start + 66] = mu.Rect.init(48, 17, 7, 17);
    value[font_start + 67] = mu.Rect.init(55, 17, 7, 17);
    value[font_start + 68] = mu.Rect.init(111, 0, 8, 17);
    value[font_start + 69] = mu.Rect.init(0, 35, 6, 17);
    value[font_start + 70] = mu.Rect.init(6, 35, 6, 17);
    value[font_start + 71] = mu.Rect.init(119, 0, 8, 17);
    value[font_start + 72] = mu.Rect.init(18, 17, 8, 17);
    value[font_start + 73] = mu.Rect.init(63, 68, 3, 17);
    value[font_start + 74] = mu.Rect.init(66, 68, 3, 17);
    value[font_start + 75] = mu.Rect.init(62, 17, 7, 17);
    value[font_start + 76] = mu.Rect.init(12, 51, 6, 17);
    value[font_start + 77] = mu.Rect.init(28, 0, 10, 17);
    value[font_start + 78] = mu.Rect.init(67, 0, 9, 17);
    value[font_start + 79] = mu.Rect.init(76, 0, 9, 17);
    value[font_start + 80] = mu.Rect.init(69, 17, 7, 17);
    value[font_start + 81] = mu.Rect.init(85, 0, 9, 17);
    value[font_start + 82] = mu.Rect.init(76, 17, 7, 17);
    value[font_start + 83] = mu.Rect.init(18, 51, 6, 17);
    value[font_start + 84] = mu.Rect.init(24, 51, 6, 17);
    value[font_start + 85] = mu.Rect.init(26, 17, 8, 17);
    value[font_start + 86] = mu.Rect.init(83, 17, 7, 17);
    value[font_start + 87] = mu.Rect.init(38, 0, 10, 17);
    value[font_start + 88] = mu.Rect.init(90, 17, 7, 17);
    value[font_start + 89] = mu.Rect.init(30, 51, 6, 17);
    value[font_start + 90] = mu.Rect.init(36, 51, 6, 17);
    value[font_start + 91] = mu.Rect.init(69, 68, 3, 17);
    value[font_start + 92] = mu.Rect.init(124, 51, 4, 17);
    value[font_start + 93] = mu.Rect.init(72, 68, 3, 17);
    value[font_start + 94] = mu.Rect.init(42, 51, 6, 17);
    value[font_start + 95] = mu.Rect.init(15, 68, 4, 17);
    value[font_start + 96] = mu.Rect.init(48, 51, 6, 17);
    value[font_start + 97] = mu.Rect.init(54, 51, 6, 17);
    value[font_start + 98] = mu.Rect.init(97, 17, 7, 17);
    value[font_start + 99] = mu.Rect.init(0, 52, 5, 17);
    value[font_start + 100] = mu.Rect.init(104, 17, 7, 17);
    value[font_start + 101] = mu.Rect.init(60, 51, 6, 17);
    value[font_start + 102] = mu.Rect.init(19, 68, 4, 17);
    value[font_start + 103] = mu.Rect.init(66, 51, 6, 17);
    value[font_start + 104] = mu.Rect.init(111, 17, 7, 17);
    value[font_start + 105] = mu.Rect.init(75, 68, 3, 17);
    value[font_start + 106] = mu.Rect.init(78, 68, 3, 17);
    value[font_start + 107] = mu.Rect.init(72, 51, 6, 17);
    value[font_start + 108] = mu.Rect.init(81, 68, 3, 17);
    value[font_start + 109] = mu.Rect.init(48, 0, 10, 17);
    value[font_start + 110] = mu.Rect.init(118, 17, 7, 17);
    value[font_start + 111] = mu.Rect.init(0, 18, 7, 17);
    value[font_start + 112] = mu.Rect.init(7, 18, 7, 17);
    value[font_start + 113] = mu.Rect.init(14, 34, 7, 17);
    value[font_start + 114] = mu.Rect.init(23, 68, 4, 17);
    value[font_start + 115] = mu.Rect.init(5, 52, 5, 17);
    value[font_start + 116] = mu.Rect.init(27, 68, 4, 17);
    value[font_start + 117] = mu.Rect.init(21, 34, 7, 17);
    value[font_start + 118] = mu.Rect.init(78, 51, 6, 17);
    value[font_start + 119] = mu.Rect.init(94, 0, 9, 17);
    value[font_start + 120] = mu.Rect.init(84, 51, 6, 17);
    value[font_start + 121] = mu.Rect.init(90, 51, 6, 17);
    value[font_start + 122] = mu.Rect.init(10, 68, 5, 17);
    value[font_start + 123] = mu.Rect.init(31, 68, 4, 17);
    value[font_start + 124] = mu.Rect.init(96, 51, 6, 17);
    value[font_start + 125] = mu.Rect.init(35, 68, 4, 17);
    value[font_start + 126] = mu.Rect.init(102, 51, 6, 17);
    value[font_start + 127] = mu.Rect.init(108, 51, 6, 17);
    break :init value;
};

pub const texture: [width * height]u8 = [_]u8{
    ,
};
