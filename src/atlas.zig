const std = @import("std");
const assert = std.debug.assert;

const mu = @import("microui.zig");

pub const width = 128;
pub const height = 128;
pub const white = mu.rect(125, 68, 3, 3);
pub const text_height = 18;

const font_start = @enumToInt(mu.Icon.Expanded) + 1;

pub inline fn fullWhite() mu.Rect {
    return atlas[white];
}

pub fn getIcon(id: mu.Icon) mu.Rect {
    const index = @enumToInt(id);

    if (index >= font_start) {
        assert(false);
        return white;
    }

    return atlas[index];
}

pub fn getGlyph(char: u8) mu.Rect {
    if (mu.isUnicodeContinuation(char)) return .{};

    const index = char + font_start;
    if (index > atlas.len) return .{};

    return atlas[index];
}

pub fn getTextWidth(text: []const u8) i32 {
    var w: i32 = 0;
    for (text) |char| w += getGlyph(char).sz.x;
    return w;
}

const atlas = init: {
    var value: [133]mu.Rect = undefined;
    value[@enumToInt(mu.Icon.Close)] = mu.rect(88, 68, 16, 16);
    value[@enumToInt(mu.Icon.Check)] = mu.rect(0, 0, 18, 18);
    value[@enumToInt(mu.Icon.Collapsed)] = mu.rect(113, 68, 5, 7);
    value[@enumToInt(mu.Icon.Expanded)] = mu.rect(118, 68, 7, 5);
    value[font_start + 32] = mu.rect(84, 68, 2, 17);
    value[font_start + 33] = mu.rect(39, 68, 3, 17);
    value[font_start + 34] = mu.rect(114, 51, 5, 17);
    value[font_start + 35] = mu.rect(34, 17, 7, 17);
    value[font_start + 36] = mu.rect(28, 34, 6, 17);
    value[font_start + 37] = mu.rect(58, 0, 9, 17);
    value[font_start + 38] = mu.rect(103, 0, 8, 17);
    value[font_start + 39] = mu.rect(86, 68, 2, 17);
    value[font_start + 40] = mu.rect(42, 68, 3, 17);
    value[font_start + 41] = mu.rect(45, 68, 3, 17);
    value[font_start + 42] = mu.rect(34, 34, 6, 17);
    value[font_start + 43] = mu.rect(40, 34, 6, 17);
    value[font_start + 44] = mu.rect(48, 68, 3, 17);
    value[font_start + 45] = mu.rect(51, 68, 3, 17);
    value[font_start + 46] = mu.rect(54, 68, 3, 17);
    value[font_start + 47] = mu.rect(124, 34, 4, 17);
    value[font_start + 48] = mu.rect(46, 34, 6, 17);
    value[font_start + 49] = mu.rect(52, 34, 6, 17);
    value[font_start + 50] = mu.rect(58, 34, 6, 17);
    value[font_start + 51] = mu.rect(64, 34, 6, 17);
    value[font_start + 52] = mu.rect(70, 34, 6, 17);
    value[font_start + 53] = mu.rect(76, 34, 6, 17);
    value[font_start + 54] = mu.rect(82, 34, 6, 17);
    value[font_start + 55] = mu.rect(88, 34, 6, 17);
    value[font_start + 56] = mu.rect(94, 34, 6, 17);
    value[font_start + 57] = mu.rect(100, 34, 6, 17);
    value[font_start + 58] = mu.rect(57, 68, 3, 17);
    value[font_start + 59] = mu.rect(60, 68, 3, 17);
    value[font_start + 60] = mu.rect(106, 34, 6, 17);
    value[font_start + 61] = mu.rect(112, 34, 6, 17);
    value[font_start + 62] = mu.rect(118, 34, 6, 17);
    value[font_start + 63] = mu.rect(119, 51, 5, 17);
    value[font_start + 64] = mu.rect(18, 0, 10, 17);
    value[font_start + 65] = mu.rect(41, 17, 7, 17);
    value[font_start + 66] = mu.rect(48, 17, 7, 17);
    value[font_start + 67] = mu.rect(55, 17, 7, 17);
    value[font_start + 68] = mu.rect(111, 0, 8, 17);
    value[font_start + 69] = mu.rect(0, 35, 6, 17);
    value[font_start + 70] = mu.rect(6, 35, 6, 17);
    value[font_start + 71] = mu.rect(119, 0, 8, 17);
    value[font_start + 72] = mu.rect(18, 17, 8, 17);
    value[font_start + 73] = mu.rect(63, 68, 3, 17);
    value[font_start + 74] = mu.rect(66, 68, 3, 17);
    value[font_start + 75] = mu.rect(62, 17, 7, 17);
    value[font_start + 76] = mu.rect(12, 51, 6, 17);
    value[font_start + 77] = mu.rect(28, 0, 10, 17);
    value[font_start + 78] = mu.rect(67, 0, 9, 17);
    value[font_start + 79] = mu.rect(76, 0, 9, 17);
    value[font_start + 80] = mu.rect(69, 17, 7, 17);
    value[font_start + 81] = mu.rect(85, 0, 9, 17);
    value[font_start + 82] = mu.rect(76, 17, 7, 17);
    value[font_start + 83] = mu.rect(18, 51, 6, 17);
    value[font_start + 84] = mu.rect(24, 51, 6, 17);
    value[font_start + 85] = mu.rect(26, 17, 8, 17);
    value[font_start + 86] = mu.rect(83, 17, 7, 17);
    value[font_start + 87] = mu.rect(38, 0, 10, 17);
    value[font_start + 88] = mu.rect(90, 17, 7, 17);
    value[font_start + 89] = mu.rect(30, 51, 6, 17);
    value[font_start + 90] = mu.rect(36, 51, 6, 17);
    value[font_start + 91] = mu.rect(69, 68, 3, 17);
    value[font_start + 92] = mu.rect(124, 51, 4, 17);
    value[font_start + 93] = mu.rect(72, 68, 3, 17);
    value[font_start + 94] = mu.rect(42, 51, 6, 17);
    value[font_start + 95] = mu.rect(15, 68, 4, 17);
    value[font_start + 96] = mu.rect(48, 51, 6, 17);
    value[font_start + 97] = mu.rect(54, 51, 6, 17);
    value[font_start + 98] = mu.rect(97, 17, 7, 17);
    value[font_start + 99] = mu.rect(0, 52, 5, 17);
    value[font_start + 100] = mu.rect(104, 17, 7, 17);
    value[font_start + 101] = mu.rect(60, 51, 6, 17);
    value[font_start + 102] = mu.rect(19, 68, 4, 17);
    value[font_start + 103] = mu.rect(66, 51, 6, 17);
    value[font_start + 104] = mu.rect(111, 17, 7, 17);
    value[font_start + 105] = mu.rect(75, 68, 3, 17);
    value[font_start + 106] = mu.rect(78, 68, 3, 17);
    value[font_start + 107] = mu.rect(72, 51, 6, 17);
    value[font_start + 108] = mu.rect(81, 68, 3, 17);
    value[font_start + 109] = mu.rect(48, 0, 10, 17);
    value[font_start + 110] = mu.rect(118, 17, 7, 17);
    value[font_start + 111] = mu.rect(0, 18, 7, 17);
    value[font_start + 112] = mu.rect(7, 18, 7, 17);
    value[font_start + 113] = mu.rect(14, 34, 7, 17);
    value[font_start + 114] = mu.rect(23, 68, 4, 17);
    value[font_start + 115] = mu.rect(5, 52, 5, 17);
    value[font_start + 116] = mu.rect(27, 68, 4, 17);
    value[font_start + 117] = mu.rect(21, 34, 7, 17);
    value[font_start + 118] = mu.rect(78, 51, 6, 17);
    value[font_start + 119] = mu.rect(94, 0, 9, 17);
    value[font_start + 120] = mu.rect(84, 51, 6, 17);
    value[font_start + 121] = mu.rect(90, 51, 6, 17);
    value[font_start + 122] = mu.rect(10, 68, 5, 17);
    value[font_start + 123] = mu.rect(31, 68, 4, 17);
    value[font_start + 124] = mu.rect(96, 51, 6, 17);
    value[font_start + 125] = mu.rect(35, 68, 4, 17);
    value[font_start + 126] = mu.rect(102, 51, 6, 17);
    value[font_start + 127] = mu.rect(108, 51, 6, 17);
    break :init value;
};

pub const texture: [width * height]u8 = [_]u8{
    ,
};
